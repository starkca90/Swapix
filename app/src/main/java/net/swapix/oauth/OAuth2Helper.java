...
//		}
    ...
    public void retrieveAndStoreAccessToken(String authorizationCode) throws IOException {
        ...
        TokenResponse tokenResponse = flow.newTokenRequest(authorizationCode).setRedirectUri(oauth2Params.getRederictUri()).execute();
        ...
        Log.i(Constants.TAG, "API Call : " + executeApiCall());
    }


    public String executeApiCall() throws IOException {
        Log.i(Constants.TAG, "Executing API call at url " + this.oauth2Params.getApiUrl());
        HttpRequest temp = HTTP_TRANSPORT.createRequestFactory(loadCredential()).buildGetRequest(new GenericUrl(this.oauth2Params.getApiUrl()));

        return temp.execute().parseAsString();
